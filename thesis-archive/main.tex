...
\begin{document}
    ...
    % Add "APPENDICES" title on same page as first appendix (required by Graduate Education)
    \clearpage
    \thispagestyle{plain}%
    \vspace*{50pt}%
    {\centering\bfseries\MakeUppercase{Appendices}\par}%
    \vspace{24pt}%
    % Now include first appendix - its \chapter command will NOT start new page
    % because we already did \clearpage
    \makeatletter
