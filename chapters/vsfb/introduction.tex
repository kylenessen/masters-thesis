...
To establish appropriate comparison baselines, we will analyze equivalent 14-day periods from the same sites and season that did not follow wind threshold events. This matched control approach ensures we distinguish wind-induced abandonment from natural fluctuations in site occupancy.
...
# Compare to control periods without wind events
model_h3_control <- glmer(site_occupied ~ days_in_period + (1|view) + (1|labeler),
                         family = binomial,
                         data = control_period_data)
\end{verbatim}
