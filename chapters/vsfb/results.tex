...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{supplemental/results/thesis_exports/figures/partial_effects_1x4_display.png}
\caption{Partial effects of the four significant predictors on monarch abundance change. From left to right: butterflies in direct sun, previous butterfly count, time since sunrise (minutes), and temperature (°C). Solid lines show the estimated smooth functions with 95\% confidence intervals (shaded regions). All effects are shown on the same scale for comparison.}\label{fig:partial_effects}
\end{figure}
